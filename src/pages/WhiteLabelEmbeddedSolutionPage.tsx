...
export function WhiteLabelEmbeddedSolutionPage(){
  ...
  <section className="goal-section wle-challenge-section"><div className="goal-shell"><Head n="" title="保留品牌与客户关系，快速补齐财富能力" copy="根据现有渠道、目标用户与运营方式，选择适合的终端和系统连接方式。"/><div className="wle-challenges">{[['无需重建完整系统','复用现有品牌、登录体系与用户入口，减少重复建设。'],['保持连续客户体验','让开户、产品、交易与持仓进入用户熟悉的渠道。'],['灵活连接现有基础','可根据项目连接现有柜台、FinOne、FinMix 及相关系统。'],['明确运营与合规边界','根据接入模式确定品牌展示、账户关系、运营责任与必要披露。']].map(([title,copy])=><article key={title}><h3>{title}</h3><p>{copy}</p></article>)}</div></div></section>
