...
export function WhiteLabelEmbeddedSolutionPage(){
  ...
  <section className="goal-section wle-challenge-section"><div className="goal-shell"><Head n="" title="在保留品牌与客户关系的同时，快速补齐财富能力" copy="根据现有渠道、目标用户与运营方式，选择适合的终端和系统连接方式。"/><div className="wle-challenges">{[['无需重建完整系统','复用现有品牌、登录体系与用户入口，减少重复建设。'],['保持连续客户体验','让开户、产品、交易与持仓进入用户熟悉的渠道。'],['灵活连接现有基础','可根据项目连接现有柜台、FinOne、FinMix 及相关系统。'],['明确运营与合规边界','根据接入模式确定品牌展示、账户关系、运营责任与必要披露。']].map(([title,copy],i)=><article key={title}><span>0{i+1}</span><h3>{title}</h3><p>{copy}</p></article>)}</div></div></section>
  ...
  <section className="goal-section goal-soft"><div className="goal-shell"><Head n="06" title="品牌体验与法律主体同时清晰"/><div className="wle-rules">{['白标展示合作机构品牌，实际金融服务和账户主体仍须符合法律关系。','嵌入体验保持合作机构品牌，必要环节明确星路相关服务主体。','不统一承诺所有客户、所有柜台支持同一开户模式。','不将开发中的股票能力写成已上线，产品范围按交付模式分别描述。'].map((x,i)=><article key={x}><span>0{i+1}</span><p>{x}</p></article>)}</div><div className="wle-cases"><article><small>已上线·待授权补充</small><h3>陆浦财富 App</h3><p>授权素材、上线范围、产品范围和用户功能待补充。</p></article><article><small>待补充</small><h3>Embedded H5</h3><p>真实客户案例待补充。</p></article><article><small>待补充</small><h3>企业财富 Web</h3><p>跨境支付客户案例及公开授权待确认。</p></article></div></div></section>
